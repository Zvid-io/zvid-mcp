...
| Tool                                                                                                                                  | Description                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `plan_creative_video`                                                                                                                 | Plan-aware storyboard and art direction from a brief. Supports `consistent`, `fresh`, and `explore` modes, recent-asset exclusions, brand tokens, style packs, scene recipes, and the no-template fallback workflow                                          |
...
Recommended creative-first flow for an AI client authoring a video:

1. `plan_creative_video` with the brief, format, duration, brand and variation mode.
2. Search `examples` with `search_creative_library`; inspect previews and exclude `recentAssetSlugs` for fresh/explore work.
3. If no complete example is genuinely close, search `design-templates`, `canvas-presets`, and `shapes` and assemble the planned scene recipes. Never force an unrelated full-video template.
4. Use `search_stock_media` for topic-specific visuals and music for each scene.
5. Call `get_project_schema` / `list_supported_elements` / `get_element_docs`, then compose the complete project JSON.
